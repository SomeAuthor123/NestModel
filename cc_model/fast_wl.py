...
def WL_both(edges, labels = None, max_iter=201):
...
def convert_labeling(labeling):
    ...
    mapping = np.empty_like(labeling)
    max_val = labeling.max()
    mapping[:] = max_val+1
    num_labels = 0
    for i in range(len(labeling)):
        val = labeling[i]
        if mapping[val] <= max_val:
...
@njit([(uint32[:], uint32[:], uint32[:], uint32), (uint32[:], uint32[:], uint64[:], uint32)])

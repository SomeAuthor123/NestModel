...
def WL(g, k_max=30, use_components=False, verbosity=0, return_meanings=False, add_labelings=False):
    ...
    if is_nx:
        ...
        if use_components:
            component_colors,_ = gt.label_components(g)
    ...
    if add_labelings:
        if is_nx:
            add_labelings_nx(g, abelings=labelings, verbosity=verbosity)
        else:
...
def get_edge_block_memberships(g : gt.Graph, colors) -> Counter:
